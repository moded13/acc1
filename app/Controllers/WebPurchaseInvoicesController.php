...
class WebPurchaseInvoicesController
{
    ...
    public function index(): void
    {
        ...
        echo "<div><div class='h1'>فواتير المشتريات</div><div class='sub'>إنشاء وعرض فواتير المشتريات.</div></div>";
        echo "<div class='row' style='margin-top:12px'>
                <a class='btn' href='" . View::e($this->base . '/purchase-invoices/new') . "'>+ فاتورة جديدة</a>
              </div>";
        ...
        try {
            ...
            if (empty($rows)) {
                ...
                foreach ($rows as $r) {
                    $showUrl     = $this->base . '/purchase-invoices/show?id=' . urlencode($r['id']);
    ...
    public function newForm(): void
    {
        ...
        echo "<div><div class='h1'>فاتورة مشتريات جديدة</div></div>";

        try {
            $suppliers = (new Supplier())->getAll(true);
            $items     = (new Item())->getAll(true);

            $supOpts = "<option value=''>-- اختر المورد --</option>";
            ...
            echo "<div class='card'>
                    <form method='POST' action='" . View::e($this->base . '/purchase-invoices') . "'>
                      <div class='row'>
                        <div class='field'><label>المورد *</label>
                          <select name='supplier_id' required>$supOpts</select>
                        </div>
                      ...
                      <table class='table' id='lines-table'>
                        <thead><tr>
                          ...
                          <th>سعر التكلفة</th><th>نسبة الضريبة</th><th>الإجمالي</th><th></th>
                        </tr></thead>
                      ...
                      <div class='row' style='margin-top:14px'>
                        ...
                        <a class='btn-outline' href='" . View::e($this->base . '/purchase-invoices') . "'>إلغاء</a>
                      </div>
                    </form>
                  </div>";
...
function addLine() {
    ...
    const itemOpts = ITEMS.map(i =>
        '<option value=\"' + i.id + '\" data-cost=\"' + i.cost_price + '\" data-vat=\"' + i.vat_rate + '\">' + i.name + '</option>'
    ).join('');
    ...
    tr.innerHTML = '<td><select name=\"item_\" onchange=\"itemSelected(this,' + lineCount + ')\" style=\"min-width:140px\">' +
        ...
        '<td><input type=\"number\" step=\"0.0001\" value=\"0\" name=\"cost_\" id=\"cost_' + lineCount + '\" oninput=\"recalc(' + lineCount + ')\" style=\"width:110px\"></td>' +
...
function itemSelected(sel, n) {
    ...
    document.getElementById('cost_' + n).value = opt.dataset.cost || 0;
...
function prepareSubmit() {
    ...
    document.querySelectorAll('#lines-body tr').forEach(tr => {
        ...
        rows.push({
            ...
            unit_cost:   document.getElementById('cost_' + n)?.value || 0,
...
addLine();
    ...
    public function store(): void
    {
        ...
        try {
            ...
            header('Location: ' . $this->base . '/purchase-invoices/show?id=' . $id);
        } catch (\Throwable $e) {
            header('Location: ' . $this->base . '/purchase-invoices/new?err=' . urlencode($e->getMessage()));
        }
        exit;
    }

    /** GET /purchase-invoices/show */
    public function show(): void
    {
        $id     = (int)($_GET['id'] ?? 0);
        $title  = 'تفاصيل فاتورة مشتريات';
        $active = 'purchases';
        require APP_PATH . '/Views/layout.php';
        ...
        try {
            ...
            echo "<div><div class='h1'>فاتورة مشتريات: " . View::e($inv['invoice_no']) . "</div>
                    <div class='sub'>التاريخ: " . View::e($inv['invoice_date']) . " | الحالة: <strong>" . View::e($statusLabel) . "</strong></div>
                  </div>";

            if ($inv['status'] === 'draft') {
                echo "<form method='POST' action='" . View::e($this->base . '/purchase-invoices/post') . "' style='margin-top:10px'>
                        ...
                        <a class='btn-outline' href='" . View::e($this->base . '/purchase-invoices') . "'>رجوع</a>
                      </form>";
            } else {
                echo "<div style='margin-top:10px'><a class='btn-outline' href='" . View::e($this->base . '/purchase-invoices') . "'>رجوع للقائمة</a></div>";
            }

            echo "<div class='card'>
                    <div class='row' style='gap:20px'>
                      <div><span class='sub'>المورد: </span><strong>" . View::e($inv['supplier_name'] ?? '') . "</strong></div>
            ...
            foreach ($lines as $l) {
                echo "<tr>
                        ...
                        <td class='num'>" . View::money($l['unit_cost']) . "</td>
